...
function AuthModal({ onClose, onSuccess, lang }) {
  ...
  const handleSendCode = () => {
    if(phone.length < 9){ setError(lang==="ar"?"رقم غير صحيح":"Invalid number"); return; }
    setError(""); setStep(1);
  };
  const handleVerify = () => {
    if(otp === "1234"){ setStep(2); setTimeout(()=>{ onSuccess({ name: lang==="ar"?"مستخدم واصي":"Wasee User", phone }); onClose(); }, 1400); }
        ...
        <div style={{padding:"24px 24px 36px"}}>
          ...
          {step===2 ? (
            <div style={{textAlign:"center",padding:"20px 0"}}>
              ...
              <div style={{fontSize:22,fontFamily:"'Playfair Display',serif",fontWeight:800}}>{T.welcomeBack}</div>
              <div style={{fontSize:13,color:C.ink3,marginTop:6}}>+963 {phone}</div>
            </div>
          ) : (
            <>
              ...
              <div style={{fontSize:13,color:C.ink2,marginBottom:24}}>
                {step===0 ? T.enterPhone : `${T.otpSent} +963 ${phone}`}
              </div>

              {step===0 ? (
                <div>
                  <div style={{display:"flex",gap:8,marginBottom:6}}>
                    <div style={{background:C.surface2,border:"1px solid "+C.border,borderRadius:12,padding:"14px 16px",fontSize:14,color:C.ink2,flexShrink:0,fontWeight:600}}>🇸🇾 +963</div>
                    <input
                      value={phone}
                  ...
                  <button onClick={handleSendCode} disabled={phone.length<9}
                    style={{width:"100%",marginTop:12,background:phone.length>=9?C.olive:C.surface2,border:"none",borderRadius:12,padding:"15px",fontSize:15,fontWeight:700,cursor:phone.length>=9?"pointer":"default",color:phone.length>=9?"#fff":C.ink3}}>
                    {T.sendCode} →
                  </button>
                </div>
              ) : (
                <div>
                  ...
                  <input
                    ...
                    onKeyUp={e=>{ if(e.target.value.length===4) handleVerify(); }}
                  />
                  ...
                  {/* Invisible tap area to trigger OTP input focus */}
  ...
  if(screen==="account") return (
    <PageBg style={{paddingBottom:100}}>
      ...
      <div style={{padding:"20px 16px 0"}}>
        ...
        <button onClick={()=>setShowAuth(true)} style={{width:"100%",background:currentUser?C.oliveBg:"#fff",border:"1px solid "+(currentUser?C.oliveBorder:C.olive),borderRadius:14,padding:"14px",color:currentUser?C.oliveDark:C.olive,fontSize:15,fontWeight:600,cursor:"pointer",marginBottom:10}}>
